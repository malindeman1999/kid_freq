...
class UnwrapPhaseMixin:
    def open_unwrap_phase_window(self) -> None:
        ...
        tk.Checkbutton(
            controls,
            text="Snap near 360*n",
            variable=self.unwrap_apply_exact_360_var,
            command=self._unwrap_on_control_changed,
        ...
        self.unwrap_status_var = tk.StringVar(value="Adjust settings, then click Update Preview.")
        ...
        tk.Button(actions, text="Close", width=12, command=self._unwrap_close).pack(side="right")
        tk.Button(actions, text="Reset View", width=12, command=self._unwrap_reset_view).pack(
            side="right", padx=(8, 0)
        )
        self.unwrap_attach_button = tk.Button(
            actions, text="Attach", width=12, command=self._unwrap_attach
        )
    ...
    def _unwrap_render(self) -> None:
        ...
        for i, scan in enumerate(scans):
            ...
            ax.plot(
                prev["freq_sorted"],
                prev["phase_corrected_initial_mod360_sorted"],
                color="lightgray",
                linewidth=0.7,
                alpha=0.9,
            ...
            ax.plot(
                prev["freq_sorted"],
                prev["phase_corrected_mod360_sorted"],
                color="darkblue",
                linewidth=0.9,
            ...
            if prev["correction_360_freqs"].size > 0:
                ax.scatter(
                    prev["correction_360_freqs"],
                    prev["correction_360_phases_mod360"],
                    s=10,
                    color="black",
                    label="360*n corrections",
                    zorder=3,
                )
            if prev["non_congruent_freqs"].size > 0:
                ax.scatter(
                    prev["non_congruent_freqs"],
                    prev["non_congruent_phases_mod360"],
                    s=16,
                    color="blue",
                    label="Non-congruent irregular",
                    zorder=4,
                )
            if prev["congruent_freqs"].size > 0:
                ax.scatter(
                    prev["congruent_freqs"],
                    prev["congruent_phases_mod360"],
                    s=20,
                    color="pink",
                    label="Congruent irregular",
                    zorder=5,
                )
            ax.set_ylabel("Phase mod 360 (deg)")
