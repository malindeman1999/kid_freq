...
class BatchResonanceFitMixin:
    def open_resonance_fit_offset_window(self) -> None:
        ...
        if not points:
            messagebox.showwarning(
                ...
                "No accepted Logan fits could be compared.\n\n"
    ...
    def _res_fit_offset_render(self) -> None:
        ...
        ax.set_title(f"Accepted Logan fits vs offset theory | reference {ref_label} ({ref_power:g} dBm)", fontsize=10)
    ...
    def open_resonance_fit_quality_window(self) -> None:
        ...
        if not ranked:
            messagebox.showwarning(
                "No batch fits",
                "No successful attached Logan marker fit results were found on the selected scans.",
            )
            return
        ...
        *,
        ...
        if fit_freq.size and fit_z.size == fit_freq.size:
            ax_amp.plot(
                ...
                alpha=alpha,
                zorder=zorder,
                label=f"{label_prefix}Logan fit",
            )
            fr0_hz = float(payload.get("fr0_hz", np.nan))
            true_fr_hz = float(payload.get("true_fr_hz", np.nan))
    ...
    def _res_fit_quality_attach_acceptance(self, *, threshold_rank: int) -> None:
        ...
        self._log(f"Attached Logan marker fit acceptance flags: {accepted_count} accepted, {rejected_count} rejected.")
    ...
    def _batch_res_fit_fit_one(self, row: dict, marker: dict, df_over_f: float) -> dict:
        ...
        payload = self._res_make_logan_fit_payload(
            ...
            f_fit=f_fit,
            success=True,
            message="Batch Logan nonlinear IQ fit complete.",
        )
        payload.update(
            {
    ...
    def _batch_res_fit_render_selected(self) -> None:
        ...
        if isinstance(payload, dict) and payload.get("success"):
            ...
            if fit_freq.size and fit_z.size == fit_freq.size:
                ...
                ax_iq.plot(np.real(fit_z), np.imag(fit_z), color="darkorange", linestyle="--", label="Logan fit")
                if np.isfinite(fr0_hz):
    ...
    def _batch_res_fit_confirm_attach(self) -> None:
        ...
        self._refresh_status()
        self._autosave_dataset()
        self._log(f"Attached {success_count} Logan marker fit result(s) across {attached_scans} scan(s).")
